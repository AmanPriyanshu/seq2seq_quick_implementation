...
encoder_user_input_data = np.zeros(
    (len(user_input), max_encoder_seq_length, num_encoder_tokens),
    dtype='float32')

for i, input_text in enumerate(user_input):
    for t, char in enumerate(input_text):
        encoder_user_input_data[i, t, input_token_index[char]] = 1

decoded_sentence = decode_sequence(encoder_user_input_data)
